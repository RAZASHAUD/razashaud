...
// Offline answer path: keyword overlap against the curated FAQ. This is what
// runs when no API key is configured, so the widget is never dead.
  ...
  const send = async (text) => {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: next.filter((m) => m.role !== 'system').slice(-8) }),
      });
      if (!res.ok) throw new Error('unavailable');
      const data = await res.json();
      setMessages([...next, { role: 'assistant', content: data.text || localAnswer(q) }]);
    } catch {
